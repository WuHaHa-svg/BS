  ...
  fetch(url)
    ...
    .then(data => {
      if (data === "[]") {
        ...
        noDataRow.innerHTML = '<td colspan="12">暂无数据</td>';
        ...
        for (const item of jsonData) {
          ...
          <td><button id="task-res" onclick="ResDetail(this)">查看检测详情</button></td>
        `;
        // 
          // <td><button id="task-res" onclick="XSSRes(this)">查看XSS注入检测详情</button></td>
    ...
    .catch(error => {
      ...
      const noDataRow = document.createElement("tr");
      noDataRow.className = "no-data";
      noDataRow.innerHTML = '<td colspan="12">请求失败！</td>';
      taskTable.tBodies[0].appendChild(noDataRow);
    });
}
GetTasks()
