...
// 获取弹窗关闭按钮元素
const closeIframe = detail.querySelector(".close-detail");
...
function getRowObj(button) {
  ...
  // 发送 POST 请求，获取详细任务数据
  fetch("http://127.0.0.1:8000/SQL/GetInfo/", {
    method: "POST",
    headers: {
    ...
    .then((data) => {
      ...
      const sqlScan = document.getElementById('sqlScan');
      sqlScan.innerHTML = `${data[0].fields.is_sql_scan === "Y" ? "是" : "否"}`;
      ...
      const statusSql = document.getElementById('statusSql');
      statusSql.innerHTML = `${data[0].fields.status_sql_scan === 'INIT' ? '刚生成' : data[0].fields.status_sql_scan === 'RECV' ? '处理中' : '已完成'}`;
